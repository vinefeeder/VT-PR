...
def get_cdm(service, profile=None, cdm_name=None):
    ...
    if "sl2000" or "sl3000" in cdm_name.lower():
        
        device_pr = Device.load(os.path.join(directories.devices, f"{cdm_name}.prd"))
        return device_pr
    try:
...
def dl(ctx, profile, cdm, *_, **__):
    ...
    try:
        ...
        raise log.exit(f" - {e}")
    device_name = device.group_certificate.get_name() if "sl3000" or "sl2000" in device.get_name() else device.system_id
    log.info(f" + Loaded {device.__class__.__name__}: {device_name} (L{device.security_level})")
    cdm = CdmPr.from_device(device) if "sl3000" or "sl2000" in device.get_name() else Cdm(device)
                ...
                if not track.key:
                    ...
                    try:
                        if "common_privacy_cert" in dir(ctx.obj.cdm):
                            ...
                            log.info(f"CDM Session ID - {session_id.hex()}")
                            wrm_header = PSSH(track.pssh).get_wrm_headers(downgrade_to_v4=True)[0] # Downgrade to v4 is necessary for ATVP
                    ...
                    try:
                        ...
                        raise log.exit(f" - Failed to close CDM session: {e}")
